...
- 继承函数的查找顺序问题
    - 优先查找自己
    - 没有查找父类
    - 构造函数如果本类没有定义，则优先查找调用父类构造函数
    - 如果本类有定义，则不在向上查找
- 构造函数
    - 是一种特殊的函数，在类初始化之前就调用了。
    - 如果定义了构造函数，则实例化是使用构造函数，不往上查找
    - 如果没有定义，父类的构造函数带参数，则构造对象时的参数应安装父类构造
- super
    - super不是关键字，而是一个类
    - super的作用是获取MRO 列表中的第一个类
    - super于父类没有什么关系，通过super可以电影到父类
...
- 菱形继承，钻石继承的问题
    ...
    - 关于多继承的MRO
        ...
        - Python本身采用C3算法来多继承继承的菱形继承进行计算的结果
            - 子类永远在父类前面
            - 如果多个父类，则根据继承语法中括号的内容的书写方式存放
            - 如果多个类继承了同一个父类，子孙类中只会选取继承语法括号中第一个父亲的父类
            
- 构造函数
    - 在对象进行实例化的时候，系统自动调用的一个函数称为构造函数，通常这个函数进行初始化操作
